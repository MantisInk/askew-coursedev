 ...
 * <p>
 * This version of askew.GameCanvas only supports both rectangular and polygonal Sprite
...
public class GameCanvas {
    ...
    public void dispose() {
        if (active != DrawPass.INACTIVE) {
            Gdx.app.error("askew.GameCanvas", "Cannot dispose while drawing active", new IllegalStateException());
            return;
        }
        spriteBatch.dispose();
        spriteBatch = null;
    ...
    public void draw(Texture image, float z) {
        if (active != DrawPass.STANDARD) {
            Gdx.app.error("askew.GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
            return;
        }
        ...
        // Unlike Lab 1, we can shortcut without a master drawing method
        spriteBatch.setColor(Color.WHITE);
        spriteBatch.draw(image, 0, 0, z, getWidth(), getHeight());
    }
    ...
    public void drawTextAlignedRight(String text, BitmapFont font, float x, float y, Color c) {
        if (active != DrawPass.STANDARD) {
            Gdx.app.error("askew.GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
            return;
        }
        GlyphLayout layout = new GlyphLayout(font, text, c, 0, Align.right, false);
        font.draw(spriteBatch, layout, x, y);
    }
    ...
    public void drawPhysics(PolygonShape shape, Color color, float x, float y) {
        if (active != DrawPass.DEBUG) {
            Gdx.app.error("askew.GameCanvas", "Cannot draw without active beginDebug()", new IllegalStateException());
            return;
        }
